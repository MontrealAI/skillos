...
def monotonic_checks_pass(proof: dict[str, Any]) -> bool:
    ...
    return isinstance(checks, dict) and bool(checks) and all(truthy(v) for v in checks.values())
...
def metric_comparison_pass(proof: dict[str, Any]) -> bool:
    ...
    if sum(bool(x) for x in comparisons) >= 3:
        return True
    ...
    ok = 0
    for key, threshold in top_level_checks:
        value = proof.get(key)
...
def proof_passed(proof: dict[str, Any]) -> bool:
    return explicit_pass_flag(proof) or monotonic_checks_pass(proof) or metric_comparison_pass(proof)
