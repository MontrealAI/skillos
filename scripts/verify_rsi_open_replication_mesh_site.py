...
def main() -> None:
    ...
    for phrase in ["Can the world replay the proof?", "Open replication", "signed receipts", "Run / regenerate"]:
    ...
    tree = receipt.get("replica_summary", {}).get("receipt_tree", {})
    if not tree.get("root"):
        fail("public receipt missing receipt tree root")
